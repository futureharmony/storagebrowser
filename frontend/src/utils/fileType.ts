...
export function detectFileType(filename: string): string {
  ...
  // 文本类型
  const textExts = ['.txt', '.md', '.log', '.ini', '.cfg', '.conf', '.json', '.xml', '.yaml', '.yml', '.csv'];
  if (textExts.includes(ext)) return 'text';
  
  // 代码类型
  const codeExts = ['.js', '.ts', '.jsx', '.tsx', '.vue', '.html', '.css', '.scss', '.less', '.py', '.java', '.cpp', '.c', '.go', '.rs', '.php', '.rb', '.sh', '.bat', '.ps1'];
  ...
  // 压缩文件类型
  const archiveExts = ['.zip', '.rar', '.7z', '.tar', '.gz', '.bz2', '.xz', '.zst', '.cab'];

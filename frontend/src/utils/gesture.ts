...
class GestureDetector {
  ...
  constructor() {
    ...
    console.log('[GestureDetector] Screen width:', window.innerWidth, 'isMobile:', window.innerWidth <= 736);
  ...
  private setupEventListeners(): void {
    ...
    window.addEventListener('resize', () => {
      ...
      if (wasMobile !== this.isMobile) {
        console.log(`[GestureDetector] Mobile detection changed: ${wasMobile} -> ${this.isMobile}`);
      }
    });
  }
  ...
  private handleTouchStart(event: TouchEvent): void {
    if (!this.isMobile || !this.isEnabled) {
      console.log(`[GestureDetector] Ignoring touch: mobile=${this.isMobile}, enabled=${this.isEnabled}`);
      return;
    }
    
    if (event.touches.length !== 1) {
      console.log(`[GestureDetector] Ignoring multi-touch: ${event.touches.length} touches`);
      return;
    }
    ...
    console.log(`[GestureDetector] Touch started at (${this.touchStartX}, ${this.touchStartY})`, {
  ...
  private handleTouchMove(event: TouchEvent): void {
    if (!this.isMobile || !this.isEnabled || !this.touchStartX) {
      console.log(`[GestureDetector] Ignoring touch move: mobile=${this.isMobile}, enabled=${this.isEnabled}, hasStart=${!!this.touchStartX}`);
      return;
    }
    
    if (event.touches.length !== 1) {
      console.log(`[GestureDetector] Ignoring multi-touch move: ${event.touches.length} touches`);
      return;
    }
    
    // Check if touch started near left edge
    if (this.touchStartX > this.edgeThreshold) {
      console.log(`[GestureDetector] Touch started too far from edge (${this.touchStartX} > ${this.edgeThreshold})`);
      return;
    }
    ...
    console.log(`[GestureDetector] Touch move: deltaX=${deltaX}, deltaY=${deltaY}, currentX=${touch.clientX}`);
    
    // Prevent vertical swipes
    if (deltaY > this.verticalThreshold) {
      console.log(`[GestureDetector] Vertical movement too large (${deltaY} > ${this.verticalThreshold}) - rejecting`);
      return;
    }
    
    // Only handle rightward swipes
    if (deltaX < 0) {
      console.log(`[GestureDetector] Leftward swipe detected (${deltaX} < 0) - rejecting`);
      return;
    }
    ...
    if (deltaX > 10) {
      ...
      this.isSwiping = true;
      console.log(`[GestureDetector] Valid swiping detected: deltaX=${deltaX}, preventing default`);
    }
  }
  ...
  private handleTouchEnd(event: TouchEvent): void {
    if (!this.isMobile || !this.isEnabled || !this.isSwiping) {
      console.log(`[GestureDetector] Ignoring touch end: mobile=${this.isMobile}, enabled=${this.isEnabled}, swiping=${this.isSwiping}`);
      return;
    }
    ...
    console.log(`[GestureDetector] Touch ended: deltaX=${deltaX}, duration=${duration}ms, velocity=${velocity.toFixed(2)}px/ms`);
    ...
    if (deltaX > this.swipeThreshold && duration < this.maxSwipeTime) {
      ...
      console.log(`[GestureDetector] Valid left-edge-swipe-right gesture detected`);
    } else {
      console.log(`[GestureDetector] Gesture rejected: deltaX=${deltaX} <= ${this.swipeThreshold} or duration=${duration} >= ${this.maxSwipeTime}`);
    }
  ...
  setEnabled(enabled: boolean): void {
    ...
    console.log(`[GestureDetector] Gesture detection ${enabled ? 'enabled' : 'disabled'}`);
  }
